function varOutput = crossVal_DR(featsAvgN,label,classifierID,featSelectID)
...
maxF = 3; B_all = cell(maxF,runs); % select the max number of features
        ...
        for i = 1 : k
            ...
            switch classifierID
                ...
                case 2 % random forests classifier
                    ...
                    B(i).b = TreeBagger(50,trainData,trainLabels,'FBoot',0.667,'oobpred','on','Method','classification','NVarToSample','all');
                    [~,posteriors] = predict(B(i).b,testData);
                case 3 % SVM classifier
                    B(i).b = svmtrain(trainLabels,trainData, '-t 2 -c 100 -b 1 -q');
